...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck)
{
    ...
    if (BLOCK_VERSION(block.nVersion) >= 3 && CBlockIndex::IsSuperMajority(3, pindex->pprev, Params().EnforceBlockUpgradeMajority())) {
...
void static UpdateTip(CBlockIndex *pindexNew) {
    ...
    if (!IsInitialBlockDownload() && !fWarned)
    {
        ...
        {
            if (BLOCK_VERSION(pindex->nVersion) > CBlock::CURRENT_VERSION)
                ++nUpgraded;
            pindex = pindex->pprev;
        }
        if (nUpgraded > 0)
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, CBlockIndex * const pindexPrev)
{
    ...
    if (BLOCK_VERSION(block.nVersion) < 3 && CBlockIndex::IsSuperMajority(3, pindexPrev, Params().RejectBlockOutdatedMajority()))
    {
...
bool AcceptBlockHeader(const CBlockHeader& block, CValidationState& state, CBlockIndex** ppindex)
{
    ...
    if (hash != Params().HashGenesisBlock() && hash != Params().HashAirdropBlock()) {
        ...
        // Check timestamp against prev
        if (block.GetBlockTime() <= pindexPrev->GetMedianTimePast()) // FIXME MedianTime is p2p median time -- Sybil vulnerable
            return state.Invalid(error("%s : block's timestamp is too early", __func__),
                                 REJECT_INVALID, "time-too-old");
